...
# 2. Очистить и подготовить данные: привести датасеты к удобному для анализа виду, 
#обработать пропущенные и аномальные значения (пропущенные значения есть только в датасете bank-full.csv)

summary(bank_full)
#Видим, что есть отрицательные значения возраста и среднемесячного баланса.

filter(bank_full, balance <= 0)
...
# В столбце deposittype есть пропуски. Заменим NA на 'no deposit' в столбце deposittype
...
#3. Трансформировать данные. Объединить датасет с данными по маркетинговой кампании с информацией по тарифам депозитов.

bank_and_deposits <- left_join(bank_full_clean, deposits_tariff, by = 'deposittype')

#4. Провести визуальный анализ данных: построить диаграммы, показывающие результаты маркетинговой компании. 
...
# 5. Рассчитать и вывести показатели эффективности маркетинговой кампании: сколько процентов коммуникаций приводит к успеху,
...
round(mean(result1$duration) / 60, 0) # Средняя длительность успешной коммуникации 9 минут 
...
round(mean(result2$duration) / 60, 0) # Средняя длительность успешной коммуникации 4 минуты.
...
# Более длительные коммуникации - более успешны. После более длинных коммуникаций клиент берет продукт.

## 6. Рассчитать средний и общий расход от депозита в случае успешной кампании:
#(на основе тарифа по формуле смоделировать сколько денег будет потрачено на выплату процентов, если:
#у человека на счету в среднем больше денег, чем требует минимальный взнос, то он положит минимальный взнос + 10% от своего среднемесячного баланса
#если депозит с возможностью снятия, то в среднем клиенты будут снимать 0,5% от остатка ежемесячно
#если депозит с возможностью пополнения, то клиенты будут класть на депозит 5% от своего среднемесячного остатка ежемесячно

# Для анализа оставим в датасете только успешные коммуникации и добавим столбец, calculation в который будем записывать результат вычислений
...
# 7. Проанализировать, какие клиенты берут новый продукт на основе маркетинговой кампании 
# (составить портрет клиента: его профессию, средний возраст, семейное положение)
...
# 8. Проанализировать, влияет ли наличие тех или иных продуктов на решение клиента взять депозит 
# (построить корреляцию между наличием у клиента ЗП проекта, кредита и проч. и его решением, предварительно преобразовав бинарные переменные в числовые для корреляции)


# Выберем нужные переменные и преобразуем бинарные переменные в числовые ( yes = 1, no = 0) 
